...
// Work around a Next 15 dev manifest bug by client-only loading heavy modals
...
export default function IndexHubClient() {
  ...
  const filteredIndexes = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "24h Change":
          return (
            parseFloat(b.sinceInceptionReturnPct?.replace(/[^-\d.]/g, "") || "0") -
            parseFloat(a.sinceInceptionReturnPct?.replace(/[^-\d.]/g, "") || "0")
          );
        case "1W Change":
          return (
            parseFloat(b.sinceInceptionReturnPct?.replace(/[^-\d.]/g, "") || "0") -
  ...
  return (
    <div className="w-full min-h-[70vh]">
      <div className="ui-scale">
        ...
        <div className="py-6">
          {loading ? (
            <div className="flex items-center justify-center min-h-[40vh] text-[color:var(--color-muted-foreground)]">
              Loading indexes...
            </div>
          ) : filteredIndexes.length === 0 ? (
            <div className="flex flex-col items-center justify-center min-h-[40vh] text-center gap-6">
              <div className="space-y-2">
                <h3 className="text-white text-xl font-semibold">No indexes yet</h3>
                <p className="text-sm">
                  {indexes.length === 0
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredIndexes.map((index) => (
                <div
                  key={index.id}
                  className="glass-card rounded-[12px] p-6 hover:bg-[color:var(--color-card)]/90 transition-colors cursor-pointer"
                  onClick={() => {
