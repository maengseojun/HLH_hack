...
export default function RedeemModal({ open, onClose, index }: Props) {
  ...
  const nav = useMemo(() => {
    // Try to extract actual numeric value from currentValue or nav
    const value = index.currentValue ?? index.nav;
    if (typeof value === 'string') {
      // Extract number from strings like "1,000.00 HYPE" or "$1,000.00"
        ...
        <div className="flex gap-2 justify-end">
          ...
          <button 
            ...
            onClick={() => {
              ...
              // Mock redeem - update localStorage
              const existingIndexes = JSON.parse(localStorage.getItem('mock-indexes') || '[]');
              const updatedIndexes = existingIndexes.map((idx: any) => {
                if (idx.id === index.id) {
                  return {
                    ...idx,
                    status: "Redeemed",
                    redemption: {
                      redeemedAmount: amountPct + "%",
                      redeemedValueUSDC: expectedRefund.net.toFixed(2),
                      fees: "0.5%",
                      slippage: "0.1%",
                      openedAt: idx.createdAt,
                      closedAt: new Date().toISOString(),
                      perAsset: idx.composition?.map((comp: any) => ({
                        symbol: comp.symbol,
                        allocationPct: comp.percentage,
                        buyPrice: "100.00",
                        sellPrice: (Math.random() * 20 + 90).toFixed(2), // Mock sell price
                        position: comp.side,
                        absReturn: (Math.random() * 10 - 5).toFixed(2),
                        pctReturn: (Math.random() * 10 - 5).toFixed(2) + "%"
                      }))
                    }
                  };
                }
              ...
              localStorage.setItem('mock-indexes', JSON.stringify(updatedIndexes));
              
              // Trigger storage event for other components to update
              window.dispatchEvent(new StorageEvent('storage', {
                key: 'mock-indexes',
                newValue: JSON.stringify(updatedIndexes)
              }));
              
              console.log("Mock index redeemed:", index.name);
              onClose();
            }}
            className="rounded-[12px] bg-red-500/90 text-white px-5 py-2 disabled:opacity-50 hover:bg-red-600 disabled:hover:bg-red-500/90"
          >
            Redeem
          </button>
        </div>
